...
async function main() {
  ...
  const sql = async <T>(text: string): Promise<T[]> => (await pool.query(text)).rows as T[];
  ...
  const vectorCols = await sql<{ table_name: string; column_name: string; udt_name: string }>(`
  ...
  const migrations = await sql<{ id: number; hash: string; created_at: string | number }>(`
