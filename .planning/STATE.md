...
| 260425-kg7 | 收藏 Tab 匿名时弹登录 modal（不再 server redirect 弹回精选）：FeedTabs → Client Component + isAuthenticated prop（false 时 收藏 渲染为 button dispatch open-login-modal，true 时仍 Link 到 /favorites）+ FeedTopBar 透传 + 三个 reader 页面 session 派生 + 6 单测 | 2026-04-25 | 2eb8023 | [260425-kg7-favorites-tab-anon-login-modal](./quick/260425-kg7-favorites-tab-anon-login-modal/) |
