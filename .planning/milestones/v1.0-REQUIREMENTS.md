...
**Coverage:** 74/75 satisfied; 1 partial (ADMIN-04 — see note below).

Checkbox state reflects verified reality per `.planning/milestones/v1.0-MILESTONE-AUDIT.md` 3-source cross-reference. The original REQUIREMENTS.md traceability table was stale on 10 items at archive time — those are flipped below to match VERIFICATION.md evidence.

For next milestone, a fresh REQUIREMENTS.md is created via `/gsd-new-milestone`.
...
- [x] **FEED-10**: Redis feed cache (5-min TTL) invalidated when a new cluster refresh completes *(code wired: `refresh-clusters.ts:35` → `invalidateFeedCache()`; operational note: requires REVALIDATE_SECRET + NEXT_PUBLIC_SITE_URL in Trigger.dev vault)*
...
- [x] **ADMIN-02**: 信源 list view with source name, URL, weight, active toggle, last-fetched, consecutive errors
- [x] **ADMIN-03**: Admin can create a new source (RSSHub route or raw RSS URL) with weight and category
- [~] **ADMIN-04**: Admin can edit source weight, name, active state *(partial — WR-02: edit form silently ignores unchecked 'active' checkbox; row-level toggle button is the workaround)*
- [x] **ADMIN-05**: Admin can soft-delete a source (items preserved, source marked inactive)
- [x] **ADMIN-06**: 信源 health indicator: red when `consecutive_empty_count ≥ 3` or `consecutive_error_count ≥ 3`
- [x] **ADMIN-07**: 用户 list view with email, provider, created-at, role, ban toggle
- [x] **ADMIN-08**: Admin can ban a user (revokes sessions, blocks interactions)
- [x] **ADMIN-09**: Admin dashboard shows daily Claude token cost (input / cache-read / cache-write / output) from `pipeline_runs`
...
- [x] **OPS-03**: Admin UI exposes dead-letter items with retry action *(single + bulk retry; CR-01 raw-SQL bug fixed in 56e82cf → `inArray(items.id, ids)`)*
...
| ADMIN-04 | Phase 6 | Partial (WR-02) |
...
- Mapped to phases: 75
- Unmapped: 0
- Satisfied at ship: 74 (Complete)
- Partial at ship: 1 (ADMIN-04 — WR-02 warning, non-blocker)

---
*Requirements defined: 2026-04-17*
*Shipped: 2026-04-24 as v1.0 MVP*
*Last updated: 2026-04-24 at milestone archive — checkboxes flipped to match VERIFICATION.md evidence per v1.0-MILESTONE-AUDIT.md 3-source cross-reference*
