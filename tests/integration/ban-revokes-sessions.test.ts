 ...
 * session rows and sets the audit columns against a real Supabase test database.
 *
 * This test builds its OWN Drizzle client via node-postgres Pool — the same
 * driver `src/lib/db/client.ts` uses in production — because our repo's
 * banUserCore relies on `db.transaction(tx => ...)`. Matching the Pool driver
 * here is what makes the integration test a faithful proof of the production
 ...
 *
 * Runs against a real Supabase database ONLY. Skips when DATABASE_URL is the
 * vitest placeholder (localhost) — detected by looking for `.supabase.` in the
 * URL or by the RUN_INTEGRATION_DB=1 opt-in. Fail-closed on production: if the
 * URL contains "prod" the Pool constructor never runs.
 */
